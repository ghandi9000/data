...
##  to dflong (calculating values using only douglas firs).  Also,
##  make a dataframe with one point for each of these values/plot/time
##  for use in self-thinning line calculations.
source('functions.R')
dflong <- read.csv("long-df.csv")
dflong <- dflong[order(dflong$install,dflong$plot, dflong$time),]
dflong <- subset(dflong, spec == "FD")
...
write.csv(dfplot, "sdp-plot-level.csv", row.names=FALSE)
